...
- **"Nothing on this set threatens it back"** when none of that member's own real, already-chosen moves clear neutral damage against the counter — a genuine wall, not just a hard hitter. Checked against the counter's own real defensive ability too (a Water Absorb holder genuinely blocks a Water move back), the same way the reasons above check the counter's own offense.
